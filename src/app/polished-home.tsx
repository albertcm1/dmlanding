...
import { audiences, credibility, integrationDetails, workflow } from "../lib/data";
...
function AuditForm({ compact = false }: { compact?: boolean }) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    const domain = site.trim().replace(/^https?:\/\//i, "").split("/")[0];
    if (!/^[a-z0-9.-]+\.[a-z]{2,}$/i.test(domain)) { setError("Enter a valid website domain to get started."); return; }
  ...
  return <form className={`candidate-audit-form ${compact ? "candidate-audit-form--compact" : ""}`} onSubmit={submit}>
    <label><Search size={19} /><span>https://</span><input aria-label="Website domain" value={site} onChange={(event) => { setSite(event.target.value); setError(""); }} placeholder={placeholder} /></label>
    <button type="submit">Start free website audit <ArrowRight size={17} /></button>
    {error && <small className="candidate-audit-error" role="alert">{error}</small>}
  </form>;
}
...
function VideoModal({ close }: { close: () => void }) {
  ...
  return <div className="candidate-video-backdrop" role="dialog" aria-modal="true" aria-label="Digimetrics product tour" onClick={(event) => event.target === event.currentTarget && close()}>
    <div className="candidate-video-modal"><button type="button" aria-label="Close video" onClick={close}><X size={20} /></button><iframe src={`https://www.youtube.com/embed/${VIDEO_ID}?autoplay=1&rel=0&modestbranding=1`} title="Digimetrics product tour" allow="accelerometer; autoplay; encrypted-media; picture-in-picture" allowFullScreen /></div>
  </div>;
}
...
function AuditWorkspace() {
  ...
  const viewDetail = activeView === "Overview" ? <div className="workspace-overview-detail"><div className="workspace-kpis"><article><span>Growth streak</span><b>7 days</b><em>3 actions completed</em></article><article><span>Tracked keywords</span><b>25</b><em>4 moved this week</em></article><article><span>Page speed</span><b>Good</b><em>2 pages to review</em></article></div><div className="workspace-bottom"><section><div className="workspace-section-head"><span className="micro-label">PRIORITY QUEUE</span><span>01 of 04</span></div><h4>{view.queue}</h4><p>{view.detail}</p><div className="issue-meta">{view.meta.map((item) => <span key={item}>{item}</span>)}</div></section><aside><div className="guide-inline"><img src="/guide-support.gif" alt="Monty" /><div><b>Monty</b><p>{view.guide}</p></div></div><a className="workspace-outline" href={PLATFORM_URL}>Open platform <ChevronRight size={16} /></a></aside></div></div> : activeView === "Site Health" ? <div className="workspace-site-health"><div className="workspace-runner"><span>https://</span><b>example.com</b><a href={PLATFORM_URL}>Run site health</a></div><div className="health-summary"><strong>78</strong><div><b>Website health</b><span>Clear priorities for the next review</span></div></div><div className="health-breakdown"><span><i className="critical" />2 critical</span><span><i className="warning" />5 warnings</span><span><i className="opportunity" />11 opportunities</span></div><div className="workspace-checklist"><span>AI crawler access <b>Ready</b></span><span>Missing page descriptions <b>Review</b></span><span>Internal-link path <b>Review</b></span></div></div> : activeView === "Tools" ? <div className="workspace-tools-detail"><div className="tool-category-pills"><span>SEO</span><span>Content</span><span>AI visibility</span><span>Strategy</span></div><div className="workspace-tool-row"><div><b>Site Health Check</b><span>Complete page, technical and AI readiness review.</span></div><a href={PLATFORM_URL}>Run</a></div><div className="workspace-tool-row"><div><b>Keyword Analysis</b><span>Find terms worth tracking by intent and opportunity.</span></div><a href={PLATFORM_URL}>Run</a></div><div className="workspace-tool-recommendation"><span>Recommended next tool</span><b>AI Discovery Audit</b><em>See how answer engines understand your site.</em></div></div> : activeView === "Projects" ? <div className="workspace-projects-detail"><div className="project-primary"><div><span>Active project</span><b>example.com</b><em>Runs, keywords and connected data in one place.</em></div><a href={PLATFORM_URL}>Open project</a></div><div className="project-stats"><span><b>4</b> recent runs</span><span><b>25</b> tracked keywords</span><span><b>1</b> connected source</span></div><div className="run-history"><span>Site Health Check <em>Ready</em></span><span>Keyword Analysis <em>Completed</em></span><span>AI Discovery Audit <em>Queued</em></span></div></div> : <div className="workspace-schedules-detail"><div className="schedule-banner"><span>Next scheduled check</span><b>Monday, 9:00 AM</b><em>Site Health Check for example.com</em></div><div className="schedule-row"><span>Rank tracking</span><b>Every Monday</b><em>Active</em></div><div className="schedule-row"><span>AI Discovery Audit</span><b>1st of each month</b><em>Active</em></div><div className="schedule-row"><span>Last completed run</span><b>Today, 10:24 AM</b><em>Ready to review</em></div></div>;
  return <figure ref={ref} className={`candidate-workspace ${visible ? "is-revealed" : ""} ${lightPreview ? "is-light-preview" : ""}`}>
    <div className="workspace-app"><aside className="workspace-side"><span className="workspace-brand">DIGIMETRICS.AI</span>{platformLinks.map(([label]) => <button type="button" key={label} onClick={() => setActiveView(label)} className={`workspace-nav ${activeView === label ? "active" : ""}`}>{label}</button>)}<div className="workspace-theme-switcher"><button type="button" aria-pressed={lightPreview} onClick={() => setLightPreview((value) => !value)} title={lightPreview ? "Theme: Light. Switch to Dark." : "Theme: Dark. Switch to Light."} aria-label={lightPreview ? "Theme: Light. Switch to Dark." : "Theme: Dark. Switch to Light."}>{lightPreview ? <Moon size={15} aria-hidden="true" /> : <Sun size={15} aria-hidden="true" />}</button></div></aside>
...
function WorkflowSurface({ step }: { step: number }) {
  const titles = ["Inspect the site", "Choose the work", "Fix with Monty", "Measure movement"];
  ...
  return <div className="workflow-surface"><div className="workflow-surface-head"><span>{String(step + 1).padStart(2, "0")}</span><b>{titles[step]}</b><em>{step === 0 ? "Site Health" : step === 1 ? "Action Plan" : step === 2 ? "Monty" : "Schedules"}</em></div><p>{copy[step]}</p><div className={`workflow-preview preview-${step}`}>{step === 0 && <><i /><i /><i /></>}{step === 1 && <><div><b>High</b><span>Improve crawl path</span></div><div><b>Next</b><span>Strengthen page proof</span></div></>}{step === 2 && <><img src="/guide-support.gif" alt="" /><span>Here is the safest first move.</span></>}{step === 3 && <><svg viewBox="0 0 240 60" aria-hidden="true"><path d="M0 48 C42 44 52 35 82 39 S126 46 150 25 S198 32 240 8" /></svg><span>Next scan: Monday, 9:00 AM</span></>}</div></div>;
}

function MonitoringPanel() {
  const [ref, cycle] = useVisibilityReplay();
  return <figure ref={ref} className={`monitor-panel monitor-panel--authentic ${cycle ? "is-revealed" : ""}`}><img src="/platform-monitoring-anonymized.png" alt="An anonymized Digimetrics platform monitoring view showing project health, scheduled runs, and tracked keywords" /><figcaption><span><i /> Live platform pattern</span><a href={PLATFORM_URL}>Open monitoring <ArrowRight size={15} /></a></figcaption></figure>;
}
...
export function PolishedHomePage() {
  ...
  return <MarketingShell><main className="candidate-home">
    <section className="candidate-hero"><DecorativeVideo className="candidate-bg-video candidate-bg-video--hero" /><div className="candidate-container"><div className="candidate-hero-copy"><span className="micro-label">DIGIMETRICS.AI</span><h1>Marketing Made Simple<br /><strong>for Everyone.</strong></h1><p>Everything You Need to Market and Grow, with an AI Concierge to Guide You.</p><AuditForm /><div className="candidate-hero-note"><i /> No credit card required. Free users get 30 credits per month.</div><a className="candidate-login-link" href={PLATFORM_URL}>Already have a workspace? Log in <ArrowRight size={15} /></a></div>
      <button type="button" className="candidate-video-preview" onClick={() => setVideoOpen(true)} aria-label="Watch Digimetrics.ai product tour"><img src={VIDEO_POSTER} alt="Digimetrics.ai product tour" /><span><Play size={20} fill="currentColor" /> Watch how Digimetrics.ai works</span></button>
    </div></section>
    ...
    <section className="candidate-section candidate-credibility"><div className="candidate-container"><div className="candidate-section-intro center"><span className="micro-label">BUILT ON MEDIAONE EXPERIENCE</span><h2>Digimetrics Has Powered Some Impressive Wins.</h2><p>Digimetrics.ai is a product by MediaOne, built for teams that want a clearer, more practical way to run digital growth.</p></div><div className="client-wordmark-rail" aria-label="Selected MediaOne clients">{[...credibility.clientNames, ...credibility.clientNames].map((name, index) => <span key={`${name}-${index}`}>{name}</span>)}</div><div className="credibility-markers">{credibility.markers.map((marker) => <span key={marker}><Check size={15} />{marker}</span>)}</div><p className="credibility-relationship">{credibility.relationship}</p></div></section>
